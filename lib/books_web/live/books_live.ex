defmodule BooksWeb.BooksLive do
  ...
  def handle_event(
        "create_book",
        %{"name" => name, "author" => author, "number" => number},
        socket
      ) do
    attrs = %{
      name: name,
  ...
  def handle_event(
    ...
    IO.inspect(attrs, label: "Updating book with attrs")
          ...
          <%= for book <- @books do %>
            <li class="p-4 flex justify-between items-center">
              <div>
                ...
                <% else %>
                  {book.number} - {book.name} <span class="text-xs">by {book.author}</span>
                <% end %>
              </div>
              <div class="flex gap-2">
                <.button
      ...
      from b in Book,
        where:
          b.library_id == ^library_id and
            (like(b.name, ^"%#{query}%") or like(b.author, ^"%#{query}%")),
        order_by: b.number
    )
  end
end
